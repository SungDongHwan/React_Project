...
const Boxmain = () => {
    ...
    for(let mv of mvList){
        console.log(mv.rank,mv.rankInten, mv.movieNm,mv.salesAmt);
        let inten = "-" ;
        if (mv.rankInten >0){inten = <span className="spup">{"▲"+mv.rankInten}</span>}
        else if (mv.rankInten <0){inten =<span className="spdown"> {"▼"+ (-mv.rankInten)} </span>}
        divtags.push(
            <div className = "rowDiv" key={mv.movieCd} onClick={()=>handleDivClick(mv)}>
                <span className="col" id="rank">{mv.rank}</span>
                <span className="col" id="rankInten">{inten}</span>
                <span className="col" id="movieNm">{mv.movieNm}</span>
                <span className="col" id="salesAmt">{parseInt(mv.salesAmt).toLocaleString('ko-KR')}</span>
            </div>
    ...
    return (
        <div className="content">
            ...
            <div className = "rowDiv0">
                <span className="col" id="rank">순위</span>
                <span className="col" id="rankInten">순위증감</span>
                <span className="col" id="movieNm">영화명</span>
                <span className="col" id="salesAmt">매출액</span>
            </div>
                {divtags}
